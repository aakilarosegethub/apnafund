...
class CampaignController extends Controller
{
     ...
     */
    public function uploadCampaignVideo()
    {
        try {
            $validator = Validator::make(request()->all(), [
     ...
     */
    public function deleteAllGallery($id)
    {
     ...
     */
    public function payRegistrationFee($slug)
    {
     ...
     */
    public function submitRegistrationFee($slug)
    {
        $campaign = Campaign::where('slug', $slug)->where('user_id', auth()->id())->firstOrFail();
        $setting = bs();
        if (empty($setting->registration_fee_enabled) || ($setting->registration_fee_min ?? 0) <= 0) {
     ...
     * Update FAQ
     */
    public function updateFaq($slug, $faqId)
    {
        $campaign = Campaign::where('slug', $slug)->firstOrFail();
     ...
     * Delete FAQ
     */
    public function deleteFaq($slug, $faqId)
    {
        $campaign = Campaign::where('slug', $slug)->firstOrFail();
     ...
     */
    public function getFaq($slug, $faqId)
    {
        $campaign = Campaign::where('slug', $slug)->firstOrFail();
     ...
     */
    public function storeUpdate($slug)
    {
        $campaign = Campaign::where('slug', $slug)->firstOrFail();
     ...
     * Update Update
     */
    public function updateUpdate($slug, $updateId)
    {
        $campaign = Campaign::where('slug', $slug)->firstOrFail();
     ...
     * Delete Update
     */
    public function deleteUpdate($slug, $updateId)
    {
        $campaign = Campaign::where('slug', $slug)->firstOrFail();
     ...
     */
    public function getUpdate($slug, $updateId)
    {
        $campaign = Campaign::where('slug', $slug)->firstOrFail();
     ...
     */
    public function removeCollaborator($slug, $userId)
    {
        try {
            $campaign = Campaign::where(function ($query) use ($slug) {
                $query->where('slug', $slug)
