    ...
    public function ipn(Request $request)
    {
        ...
        if ($charge['status'] == 'succeeded') {
            ...
            try {
                ...
                if (! $adminTemplate) {
                    \Log::warning('ADMIN_PAYMENT_SUCCESS template not found or inactive. Email not sent to admin (StripeJs).');
                } else {
                    $adminRecipients = adminMailNotifyRecipients();
                    if ($adminRecipients === []) {
                        \Log::warning('No admin email recipients (set Site email in Basic settings or add admin user emails). StripeJs.');
                    } else {
                        $shortCodes = [
                            'full_name' => $deposit->full_name,
                            'email' => $deposit->email,
                        ...
                        \Log::info('Payment success email sent to admin(s) (StripeJs): '.$toList);
                    }
                }
            } catch (\Exception $e) {
