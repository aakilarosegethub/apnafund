    ...
    public function donateNow(Request $request): JsonResponse
    {
        ...
        if ($amt > $remain_amt) {
            if ($remain_amt <= 0.01) {
                ...
                $table = 'campaigns';
                $field = ['status' => 1]; // 1 = approved/completed
                ...
                $data_values = [$uid, 'Deposite Refund Of Fund Id#'.$fund_id, 'Credit', $amt, $timestamps];
            ...
            if (($vp['wallet'] ?? 0) >= $wall_amt) {
                if ($wall_amt != 0) {
                    ...
                    $table = 'wallet_report';
                    $field_values = ['uid', 'message', 'status', 'amt', 'tdate'];
                    $data_values = [$uid, 'Wallet Used in Order Id#'.$fund_id, 'Debit', $wall_amt, $timestamps];
                ...
                $fields = [
                    ...
                    'filters' => [['field' => 'tag', 'key' => 'user_id', 'relation' => '=', 'value' => $fundata['uid']]],
                    'contents' => $content,
                    'headings' => $heading,
                ];
