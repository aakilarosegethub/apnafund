...
class UserController extends Controller
{
    public function home()
    {
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            if ($donation) {
                $report['donationAmount']->push(intval($donation->donation_amount));
            } else {
                $report['donationAmount']->push(0);
            }

            $withdraw = $monthWiseWithdraw->firstWhere('month', $monthName);

            if ($withdraw) {
                $report['withdrawAmount']->push(intval($withdraw->withdraw_amount));
            } else {
                $report['withdrawAmount']->push(0);
            }
        }
