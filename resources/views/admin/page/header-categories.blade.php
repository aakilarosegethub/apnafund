...
@section('master')
    <div class="col-12">
            ...
            <tbody>
                @forelse ($headerCategories as $headerCategory)
                    <tr>
                        ...
                        <td>
                            <span class="fw-bold">{{ $headerCategory->sort_order ?? 0 }}</span>
                        </td>
                        <td>
                            @php echo $headerCategory->statusBadge @endphp
                        </td>
                        <td>
                            <div class="d-flex justify-content-end gap-2">
                                <button type="button" class="btn btn--sm btn-outline--base editBtn" 
                                        data-resource="{{ $headerCategory }}" 
                                        data-action="{{ route('admin.header-categories.store', $headerCategory->id) }}">
                                    <i class="ti ti-edit"></i> @lang('Edit')
                                </button>

                                @if ($headerCategory->status == 'active')
                                    <button type="button" class="btn btn--sm btn--warning decisionBtn" 
                                            data-question="@lang('Are you sure to inactive this header category?')" 
                                            data-action="{{ route('admin.header-categories.status', $headerCategory->id) }}">
                                        <i class="ti ti-ban"></i> @lang('Inactive')
                                    </button>
                                @else
                                    <button type="button" class="btn btn--sm btn--success decisionBtn" 
                                            data-question="@lang('Are you sure to active this header category?')" 
                                            data-action="{{ route('admin.header-categories.status', $headerCategory->id) }}">
                                ...
                                <button type="button" class="btn btn--sm btn--danger decisionBtn" 
                                        data-question="@lang('Are you sure to delete this header category?')" 
                                        data-action="{{ route('admin.header-categories.delete', $headerCategory->id) }}">
        ...
        @if ($headerCategories->hasPages())
            {{ paginateLinks($headerCategories) }}
        @endif
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title" id="addModalLabel">@lang('Add New Header Category')</h2>
                    <button type="button" class="btn btn--sm btn--icon btn-outline--secondary modal-close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="ti ti-x"></i>
                    </button>
                </div>
                <form action="{{ route('admin.header-categories.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="row g-4">
                            <div class="col-12">
                                <label class="form--label required">@lang('Label')</label>
                                <input type="text" class="form--control" name="label" required placeholder="@lang('e.g. Art/Crafts')">
                            </div>
    ...
    <div class="custom--modal modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title" id="addModalLabel">@lang('Update Header Category')</h2>
                    <button type="button" class="btn btn--sm btn--icon btn-outline--secondary modal-close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="ti ti-x"></i>
                    </button>
                </div>
                <form action="" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="row g-4">
                            <div class="col-12">
                                <label class="form--label required">@lang('Label')</label>
                                <input type="text" class="form--control" name="label" id="editLabel" required placeholder="@lang('e.g. Art/Crafts')">
                            </div>
...
@push('breadcrumb')
    <div class="d-flex flex-wrap gap-2 align-items-center">
        ...
        <form method="GET" action="{{ route('admin.header-categories.index') }}" class="d-flex flex-wrap align-items-center gap-2">
