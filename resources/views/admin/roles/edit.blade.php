    ...
    <div class="col-12">
        <div class="custom--card">
            <div class="card-header">
                <h3 class="title">@lang('Edit Role')</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="row g-4">
                    @csrf
                    @method('PUT')
                    <div class="col-md-6">
                            ...
                            @foreach($dashboardWidgets as $key => $config)
                                <div class="col-md-6 col-lg-4">
                                    <div class="form-check">
                                        @php $roleWidgets = old('dashboard_widgets', $role->dashboard_widgets ?? []); @endphp
                                        <input type="checkbox" class="form-check-input" name="dashboard_widgets[]" value="{{ $key }}" id="widget_{{ $key }}" {{ in_array($key, (array)$roleWidgets) ? 'checked' : '' }}>
                    ...
                    <div class="col-12">
                        ...
                        <p class="text-muted small mb-2">@lang('Select which permissions this role should have.')</p>
                        <div class="row g-3">
                                            ...
                                            @foreach($modulePerms as $perm)
                                                <div class="col-md-4 col-lg-3">
                                                    <div class="form-check">
                                                        ...
                                                        <input type="checkbox" class="form-check-input" name="permissions[]" value="{{ $perm->id }}" id="perm_{{ $perm->id }}" {{ in_array($perm->id, old('permissions', $rolePermIds)) ? 'checked' : '' }}>
                    ...
                    @endif
                    <div class="col-12">
                        <button type="submit" class="btn btn--base">@lang('Update Role')</button>
