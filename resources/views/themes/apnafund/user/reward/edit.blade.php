            ...
            <div class="col-lg-8">
                <div class="content-card">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h4 class="mb-0">
                            <i class="fas fa-edit me-2"></i>@lang('Edit Reward')
                        </h4>
                    ...
                    <p class="text-muted mb-4">@lang('Update your reward details') - <strong>{{ $campaign->name }}</strong></p>
                    <form action="{{ route('user.rewards.update', [$campaign->slug, $reward->id]) }}" method="POST" enctype="multipart/form-data" id="rewardForm">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-8">
                                <div class="form-group required-field">
                                    ...
                                    <input type="text" class="form-control" id="rewardTitle" name="title" 
                                           value="{{ old('title', $reward->title) }}" placeholder="@lang('Enter an attractive title for your reward')" required>
                                    <div class="form-text">
                                        <span id="titleCounter">{{ strlen($reward->title) }} characters</span>
                                    </div>
                                    @error('title')
                            ...
                            <div class="col-md-4">
                                <div class="form-group required-field">
                                    ...
                                    <select class="form-control form-select" name="type" id="rewardType" required>
                                        ...
                                        <option value="physical" {{ old('type', $reward->type) == 'physical' ? 'selected' : '' }}>
                                            @lang('Physical Reward')
                                        </option>
                                        <option value="digital" {{ old('type', $reward->type) == 'digital' ? 'selected' : '' }}>
                                            @lang('Digital Reward')
                                        </option>
                                    </select>
                                    @error('type')
                        ...
                        <div class="form-group required-field">
                            ...
                            <textarea class="form-control" name="description" id="rewardDescription" rows="4" 
                                      placeholder="@lang('Describe what supporters will receive with this reward')" required>{{ old('description', $reward->description) }}</textarea>
                            <div class="form-text d-flex justify-content-between">
                                <span>@lang('Be specific about what supporters will get. This helps build trust and excitement.')</span>
                                <span class="text-muted" id="descriptionCounter">{{ strlen($reward->description) }} characters</span>
                            </div>
                            @error('description')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-group required-field">
                                    <label for="minimumAmount" class="form-label">@lang('Minimum Donation Amount')</label>
                                        ...
                                        <input type="number" class="form-control" name="minimum_amount" id="minimumAmount"
                                               value="{{ old('minimum_amount', $reward->minimum_amount) }}" step="0.01" min="1" 
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="quantity" id="rewardQuantity"
                                           value="{{ old('quantity', $reward->quantity) }}" min="1" 
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="terms_conditions" id="termsConditions" rows="3" 
                                      placeholder="@lang('Any special terms or conditions for this reward (optional)')">{{ old('terms_conditions', $reward->terms_conditions) }}</textarea>
                        ...
                        <div class="d-flex gap-3">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>@lang('Update Reward')
                            </button>
                            <a href="{{ route('user.rewards.index', $campaign->slug) }}" class="btn btn-outline-secondary">
                    ...
                    <div id="rewardPreview">
                        <div class="preview-card">
                            <div class="preview-image" id="previewImageArea">
                                ...
                                <img id="previewImage" src="{{ $reward->image ? getImage(getFilePath('reward') . '/' . $reward->image, getThumbSize('reward')) : '' }}" alt="Preview" style="display: {{ $reward->image ? 'block' : 'none' }}; width: 100%; height: 100%; object-fit: cover; border-radius: 10px 10px 0 0;">
                            </div>
                            <div class="preview-content">
                                <div class="preview-title" id="previewTitle">{{ $reward->title }}</div>
                                <div class="preview-description" id="previewDescription">{{ $reward->description }}</div>
                                <div class="preview-meta">
                                    <span class="preview-category" id="previewType">{{ ucfirst($reward->type) }} Reward</span>
                                    <span class="preview-amount" id="previewAmount">{{ $setting->cur_sym }}{{ showAmount($reward->minimum_amount) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            console.log('DOM loaded, initializing reward edit preview...');
