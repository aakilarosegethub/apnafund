...
<div class="payment-container">
            ...
            <div class="fundraiser-info">
                ...
                @if(request('reward') || request('reward_id'))
                    @php
                        $selectedRewardId = request('reward') ?: request('reward_id');
                        $selectedReward = $campaignData->rewards()->find($selectedRewardId);
                    @endphp
                    @if($selectedReward)
                        <div class="alert alert-info mt-2" style="background: #e7f3ff; border: 1px solid #b3d9ff; border-radius: 8px; padding: 12px; margin-top: 10px;">
            ...
            <div class="donation-section">
                <h3 class="section-title">@lang('Contribution Amount')</h3>
                @php
                    $selectedRewardId = request('reward') ?: request('reward_id');
                    $selectedReward = $campaignData->rewards()->find($selectedRewardId);
                    $isRewardSelected = $selectedReward ? true : false;
                    $rewardAmount = $selectedReward ? $selectedReward->minimum_amount : 0;
                @endphp
                ...
                @if($isRewardSelected)
                    ...
                    <div class="alert alert-warning mt-2" style="background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; padding: 12px; margin-top: 10px; font-size: 0.9rem;">
                        ...
                        <strong>@lang('Amount is Fixed')</strong> - @lang('This amount is required for the selected reward and cannot be changed')
                    </div>
                @else
                    <!-- No Reward Selected - Normal Amount Selection -->
                ...
                @if (!$authUser)
                    <div class="row g-3 mb-4">
                        <div class="col-lg-6">
                            ...
                            @php
                                $detectedCountry = $gatewayContextCountry ?? detectUserCountry() ?? 'Pakistan';
                            ...
                            <select class="form--control" name="country" id="countrySelect">
                                ...
                                @foreach ($countries as $country)
                                    <option value="{{ $country->country }}" @selected(old('country', $detectedCountry) == $country->country)>
                                        {{ $country->country }}
            ...
            <div id="personalInfoSection">
                <h3 class="section-title">@lang('Personal Information')</h3>
                    ...
                    <div class="col-lg-3 col-md-6">
                                            ...
                                            @else
                            @php
                                $userCountry = $gatewayContextCountry ?? session('user_country') ?? 'Pakistan';
                                if (empty($userCountry)) {
                                    $userCountry = 'Pakistan';
                                }
                            @endphp
                            <input type="text" class="form--control" name="country_display" value="{{ $userCountry }}" readonly>
            ...
            <div class="donation-summary">
                ...
                <div class="summary-item summary-total">
                    ...
                    <span class="summary-value d-inline-flex align-items-center">
                        <span id="totalAmount">{{ $contributeDisplaySymbol }}0.00</span>
                        @if($showRateDebug)
...
@section('script')
    <script>
        $(document).ready(function() {
            ...
            const rewardId = $('#rewardIdInput').val();
            const isRewardSelected = rewardId && rewardId !== '';
            
            // If reward is selected, auto-fill and lock amount
            if (isRewardSelected) {
                const rewardAmount = parseFloat($('#customAmount').val()) || 0;
                if (rewardAmount > 0) {
                    selectedAmount = rewardAmount;
                    $('#amountInput').val(rewardAmount.toFixed(2));
                    $('#customAmount').prop('readonly', true);
                    // Hide amount buttons if they exist
                    $('#amountButtons').hide();
                    updateSummary();
                    checkFormValidity();
                }
            ...
            // Amount button selection (only if reward not selected)
            $('.amount-btn').on('click', function() {
                if (isRewardSelected) return false; // Prevent if reward selected
            ...
            // Custom amount input (only if reward not selected)
            ...
            function checkFormValidity() {
                ...
                // If reward is selected, amount is already set and locked
                if (isRewardSelected && amount > 0 && hasSelectedGateway) {
                    $('#paymentButton').prop('disabled', false);
                } else if (!isRewardSelected && amount > 0 && hasSelectedGateway) {
                    $('#paymentButton').prop('disabled', false);
                } else {
            ...
            $('#rateDebugIcon').on('click', function(e) {
                ...
                const totalDisplay = selectedAmount + tipAmt;
                const platformDonationDbg = selectedAmount * inputToPlatformFactor;
                let html = '<strong>Display ({{ $siteCur }}):</strong> ' + totalDisplay.toFixed(2) + '<br><strong>Platform (' + platformCurrency + '):</strong> donation ' + platformDonationDbg.toFixed(4) + '<br>';
                const opt = $('.payment-option.selected');
                if (opt.length) {
                    ...
                    if (gw && gw.rate) {
                        ...
                        const charge = fixedCharge + (platformDonationDbg * pctCharge / 100);
                        const finalInGateway = (platformDonationDbg + charge) * rate;
                        html += '<strong>Gateway:</strong> ' + curr + '<br>Rate: 1 ' + platformCurrency + ' = ' + rate.toFixed(4) + ' ' + curr + '<br>Charge (platform): ' + charge.toFixed(2) + ' ' + platformCurrency + '<br><strong>You pay:</strong> ' + finalInGateway.toFixed(2) + ' ' + curr;
                    } else {
            ...
            $('#donationForm').on('submit', function(e) {
                ...
                // If reward is selected, ensure amount matches reward minimum
                if (isRewardSelected) {
                    const rewardAmount = parseFloat($('#customAmount').val()) || 0;
                    if (rewardAmount <= 0) {
                        e.preventDefault();
                        alert('@lang("Invalid reward amount")');
                        return false;
                    }
                    // Force the amount to be the reward amount
                    $('#amountInput').val(rewardAmount.toFixed(2));
                } else {
                if (amount <= 0) {
                    e.preventDefault();
